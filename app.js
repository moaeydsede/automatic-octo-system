...
// ===== Frontend Admin Bootstrap (GitHub) =====
const FRONT_ADMIN_USER = "admin";
const FRONT_ADMIN_PASS = "Admin123"; // يقبل أيضًا "Admin 123" (مسافة اختيارية)
function isFrontendAdmin(username, password){
  const u=(username||"").trim().toLowerCase();
  const p=(password||"").trim().replace(/\s+/g,""); // remove spaces
  return (u===FRONT_ADMIN_USER.toLowerCase() && p===FRONT_ADMIN_PASS.replace(/\s+/g,""));
}
...
// يعتمد على شيت ROLES_PERMS (role, perm, allowed)
async function openPermissionsAdmin(){
  try{
    // جلب البيانات الحالية
    const rolesRes = await api("list", {sheet:"ROLES"});
    const permsRes = await api("list", {sheet:"ROLES_PERMS"});
    const roles = (rolesRes.rows||[]).map(r=>String(r.role||r.name||r.Role||"").trim()).filter(Boolean);
    const rp = permsRes.rows||[];

    // تجميع الصلاحيات
    const allPerms = Array.from(new Set(rp.map(x=>String(x.perm||x.permission||x.PERM||"").trim()).filter(Boolean))).sort();
    ...
    rp.forEach(x=>{
      ...
      const perm=String(x.perm||"").trim();
      const allowed=String(x.allowed||x.value||"").trim().toUpperCase()==="TRUE";
      if(!map[role]) map[role]={};
      map[role][perm]=allowed;
    });

    const body=document.createElement("div");
    body.innerHTML = `
      <div class="muted">شاشة الصلاحيات داخل GitHub (Admin). يتم حفظها داخل Google Sheet (ROLES_PERMS).</div>
      <div class="tablewrap" style="margin-top:10px; max-height:60vh; overflow:auto">
        <table id="pt">
          <thead><tr><th>الدور</th>${allPerms.map(p=>`<th style="min-width:120px">${esc(p)}</th>`).join("")}</tr></thead>
          <tbody></tbody>
        </table>
      </div>
      <div class="muted" style="margin-top:10px">ملاحظة: بعد الحفظ، يحتاج المستخدم تسجيل خروج/دخول لتحديث الصلاحيات.</div>
    `;
    const tb=body.querySelector("tbody");

    roles.forEach(role=>{
      const tr=document.createElement("tr");
      tr.innerHTML = `<td><b>${esc(role)}</b></td>` + allPerms.map(p=>{
        const checked = !!(map[role] && map[role][p]);
        return `<td style="text-align:center">
          <input type="checkbox" data-role="${esc(role)}" data-perm="${esc(p)}" ${checked?"checked":""}>
        </td>`;
      }).join("");
      tb.appendChild(tr);
    });
    ...
    saveBtn.onclick=async()=>{
      // build CSV: role,perm,allowed
      const rows = [["role","perm","allowed"]];
      body.querySelectorAll("input[type=checkbox][data-role]").forEach(cb=>{
        const role=cb.getAttribute("data-role");
        const perm=cb.getAttribute("data-perm");
        const allowed = cb.checked ? "TRUE" : "FALSE";
        rows.push([role, perm, allowed]);
      });
      const csvText = rows.map(r=>r.map(csvEsc).join(",")).join("\n");
      // يستخدم endpoint import_csv (موجود) لاستبدال الشيت
      await api("roles_perms_set", {rows: rows.slice(1)});
      toast("تم حفظ الصلاحيات");
      closeModal();
    };

    openModal("الصلاحيات (Admin)", body, [saveBtn]);
  }catch(e){
    toast(e.message||"تعذر");
  }
}
...
window.addEventListener("load", async()=>{
  ...
  el("btnLogin").onclick=async()=>{
    ...
    try{
      let data;
      const frontAdmin = isFrontendAdmin(u,p);
      try{
        data=await api("login",{username:u,password:p});
        state.token=data.token; state.user=data.user; state.perms=data.perms; state.page="dashboard";
        sessionStorage.removeItem("frontend_admin");
        sessionStorage.removeItem("perms_override");
      }catch(e){
        if(!frontAdmin) throw e;
        // دخول محلي (واجهة) — بعض العمليات ستحتاج الاتصال بالـ API
        sessionStorage.setItem("frontend_admin","1");
        sessionStorage.setItem("perms_override","*");
        state.token=null;
        state.user={username:FRONT_ADMIN_USER, role:"Admin"};
        state.perms={};
